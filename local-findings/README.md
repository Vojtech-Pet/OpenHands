...
| 7 | [issue-07-no-per-conversation-workspace-selection.md](issue-07-no-per-conversation-workspace-selection.md) | No API to select a workspace per conversation — blocks desktop clients, multi-project UIs, parallel conversations, project isolation, multi-user servers. Proposes a server-side allow-list of named project roots (never a client-supplied path) to keep the OpenHands Cloud trust boundary intact | High — confirmed in source; standalone feature request with a security-conscious design |
