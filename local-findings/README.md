...
| 7 | [notes-planning-agent-capability-mismatch.md](notes-planning-agent-capability-mismatch.md) | The Planning agent's system prompt (as shipped) does not explicitly state that it has no terminal/git access, risking fabricated branch/commit info | Medium — observed with a **customized** planning prompt; stock prompt not independently re-checked |
| 8 | [notes-lm-studio-integration-gotchas.md](notes-lm-studio-integration-gotchas.md) | Not an OpenHands bug per se, but two sharp integration edges worth documenting for anyone running OpenHands against a local, VRAM-constrained LLM server | Informational |
